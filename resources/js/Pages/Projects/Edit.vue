        ...
        <div class="py-12">
            <div class="max-w-md mx-auto sm:px-6 lg:px-8 bg-white">
                <form class="p-4 space-y-4" @submit.prevent="submit">
                    <div>
                        <InputLabel for="skill_id" class="mb-1" value="Skill"/>
                        <select v-model="form.skill_id" id="skill_id" name="skill_id"
                                class="w-full block rounded-md pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                            <option v-for="skill in skills" :key="skill.id" :value="skill.id"
                                    v-text="skill.name"></option>
                        </select>
